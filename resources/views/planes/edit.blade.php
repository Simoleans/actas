	  	...
	  	<div class="col-md-7 col-md-offset-2">
	    	<div class="box box-success">
		      <div class="box-header with-border">
		        <h3 class="box-title"><i class="fa fa-users"></i> Editar Plan</h3>
		        <span class="pull-right"></span>
		       </div>
      			<div class="box-body">
					<form class="" action="{{ route('planes.update',['id' => $plan->id]) }}" method="POST" enctype="multipart/form-data">
					{{ method_field( 'PUT' ) }}
					{{ csrf_field() }}
						...
						<div class="row">
							...
							<div class="col-md-12">
								<div class="form-group {{ $errors->has('nombre')?'has-error':'' }}">
									...
									<input id="nombre" class="form-control" type="text" name="nombre" value="{{ $plan->nombre }}" placeholder="Plan" required>
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group {{ $errors->has('fecha_inicio')?'has-error':'' }}">
									<label class="control-label" for="fecha_inicio">Fecha Inicio: *</label>
									<input id="fecha_inicio" class="form-control date" type="text" name="fecha_inicio" value="{{ $plan->fecha_inicio }}" placeholder="Fecha Inicio" required>
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group {{ $errors->has('fecha_fin')?'has-error':'' }}">
									<label class="control-label" for="fecha_fin">Fecha Final: *</label>
									<input id="fecha_fin" class="form-control date" type="text" name="fecha_fin" value="{{ $plan->fecha_fin }}" placeholder="Fecha Final" required>
								</div>
							</div>
						</div>
							@if (count($errors) > 0)
